...
//		System.out.println(p[2].id);
		...
		String inputId,pw;
		String retry;
		boolean flag = false;//true넣어도됨 상관없음
		do {
			flag = false;
			System.out.print("id입력:");
			inputId=sc.next();
			if(inputId.length()<3||inputId.length()>8) {
				System.out.println("id는 3글자에서~8글자로 입력해야 합니다.");
				continue;
			}
			...
			for(i=0;i<p.length;i++) {	
				if(p[i].id.equals(inputId)) {
					if(p[i].pw.equals(pw)) {
						...
						flag = true;//true로 바꾸고 break만나 빠져나감
						break;
					}
					else {
...
//				}
			...
			if(retry.equalsIgnoreCase("n"))//대소문자 무시하고 비교
					break;
